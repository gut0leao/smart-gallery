#!/bin/bash
# Script para restaurar backup completo do site WordPress
# Restaura banco de dados, uploads, plugins e temas do diretório sincronizado (OneDrive)

set -e

if [ ! -d "./backups" ]; then
    echo "⚠️  Diretório ./backups não encontrado! Execute antes:"
    echo "$ ./scripts/map_backup_dir.sh /caminho/do/diretorio/dos/backups"
    exit 1
fi

# Caminho do diretório de backup (ajuste conforme necessário)
BACKUP_DIR="./backups"

# Verifica se a data foi passada como argumento
if [ -z "$1" ]; then
    echo "Uso: $0 <DATA>"
    echo "Exemplo: $0 20250827-153000"
    exit 1
fi
RESTORE_DATE="$1"

# Restaurar banco de dados
BACKUP_DB="$BACKUP_DIR/${RESTORE_DATE}-db.sql.gz"
if [ -f "$BACKUP_DB" ]; then
    echo "Restaurando banco de dados de $BACKUP_DB..."
    ddev import-db --file="$BACKUP_DB"
else
    echo "❌ Arquivo de banco de dados não encontrado para a data $RESTORE_DATE."
fi

# Restaurar uploads
BACKUP_UPLOADS="$BACKUP_DIR/${RESTORE_DATE}-uploads.tar.gz"
if [ -f "$BACKUP_UPLOADS" ]; then
    echo "🗂️  Restaurando uploads de $BACKUP_UPLOADS..."
    tar xzf "$BACKUP_UPLOADS"
else
    echo "❌ Arquivo de uploads não encontrado para a data $RESTORE_DATE."
fi

# Restaurar plugins
BACKUP_PLUGINS="$BACKUP_DIR/${RESTORE_DATE}-plugins.tar.gz"
if [ -f "$BACKUP_PLUGINS" ]; then
    echo "🗂️  Restaurando plugins de $BACKUP_PLUGINS..."
    tar xzf "$BACKUP_PLUGINS"
else
    echo "❌ Arquivo de plugins não encontrado para a data $RESTORE_DATE."
fi

# Restaurar temas
BACKUP_THEMES="$BACKUP_DIR/${RESTORE_DATE}-themes.tar.gz"
if [ -f "$BACKUP_THEMES" ]; then
    echo "🗂️  Restaurando temas de $BACKUP_THEMES..."
    tar xzf "$BACKUP_THEMES"
else
    echo " ❌ Arquivo de temas não encontrado para a data $RESTORE_DATE."
fi

echo "✅ Restauração concluída."